...
import sys
sys.stdout = open('outputPE\\output.txt', 'a', encoding='utf-8')
...
serie = fileToSerie('Petrolina-Jan-Fev.csv')

'''#-----------------
from sklearn.model_selection import train_test_split
train, valTest = train_test_split(serie, test_size=0.4, shuffle=False)
validation, test = train_test_split(valTest, test_size=0.5, shuffle=False)
print(train.count(), validation.count(), test.count())
print(train.mean(), validation.mean(), test.mean())
print(train.std(), validation.std(), test.std())
#-----------------'''
...
outputFinal.to_csv('outputPE\\saidaFinal.csv', header=True, index=True)
# plotResult(outputFinal)
allMSE.boxplot()
plt.savefig('outputPE\\boxplot.png', format='png')
